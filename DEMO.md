...
1. **Open `/pro` on a phone.** The header shows a live price + `● DEEPBOOK LIVE` and a `σ` — that's
   a real DeepBook mid — toggle **SUI**, **BTC** (XBTC/USDC), or **DEEP** — with volatility
   ...
   **[Bitcoin options ↗](https://wick-markets.vercel.app/pro?asset=XBTC_USDC)** — `/pro?asset=XBTC_USDC`,
   a real on-chain CLOB mark.) The chart is seeded with real DeepBook candle history.
...
- **Verify a round end-to-end yourself, trusting nothing** — `npm run play -- --seed 4242` runs one `/pro` round and prints its `commit`, revealed `seed`, and `paramsJson`. Confirm the path was committed before the reveal — **easiest: open [wick-markets.vercel.app/api/verify-pro](https://wick-markets.vercel.app/api/verify-pro) in any browser** (a live, self-contained page that re-hashes **client-side via Web Crypto** — nothing is sent to the server), paste the three values → **HONEST**. Or with no clone at all: `scripts/verify-pro.html` (offline file) · `POST`/`GET` `/api/verify-pro` (JSON) · the SDK's `verifyProRound(commit, seed, paramsJson)` · `npm run verify:pro-round -- --commit … --seed … --params '…'` (the CLI one-shot — paste play's three values, get **HONEST**/**TAMPERED** without leaving the terminal) · `npm run verify:pro-fairness`. Six independent surfaces, one SHA-256, same answer.
...
- **Full Move test suite green** — `cd move && sui move test` (live count on the README badge; incl. 10k seeded-path conformance vectors, TS↔Move byte-identical, enforced in CI). **Every fund-safety property maps to a named test** — [`move/SAFETY.md`](move/SAFETY.md) is the traceability map (no double-pay on any settlement path · settle-only-against-own-market · every open-path abort guard · DNT mutual-exclusion · oracle-version pinning). Run one: `sui move test <name>`.
