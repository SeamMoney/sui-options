...
> **Re-verify it yourself** (no wallet/CLI): `python3 scripts/verify-deployment.py` — checks every object is live, the vaults are funded, **and the faucet wallet has runway** (so a judge can actually fund a burner and play), straight from a public fullnode.
...
The deployed source compiles clean and passes **695 / 695 Move tests** (0 failed,
