...
Run the whole suite: `sui move test` (from `move/`) — **697/697 passing**.
...
| The rug is enforced by the ride's OWN round even after the round-roll clears the live flag — each fire is mirrored into a durable `Table<round, rug_seg>` and `decide_settlement` reads it by `ride.round_index`, so a ride RUGGED in its round that ALSO touched in-round can't escape to TOUCH_WIN by holding across the boundary (the systematic touched-then-rugged loss-of-funds residual, #694) | `rugged_ride_cannot_escape_via_in_round_touch_held_cross_round` · `post_rug_open_keeps_its_win_under_the_durable_read` (the bound is exact: a post-rug HONEST bet, entry > rug_seg, is NOT over-rugged) · `rug_history_entry_dropped_when_round_fully_settles` (the durable `Table` is storage-bounded — each round's entry is reclaimed once that round fully settles, so it can't grow without bound) (+ all 10 within-round rug tests unchanged) | `segment_market_v4_tests.move` |
| The SAME cross-round bound holds for the v2 segment-market ride (`segment_market.move`, a distinct module from v4) — its `close` touch scan is bounded to the ride's own round too, so a v2 ride held across its round boundary can't "touch-win" on a later round's segment and drain the vault (cross-round vault-drain fix, #733) | `v2_ride_cannot_touch_win_on_a_later_rounds_segment` | `segment_market_e2e_replay.move` |
| No NEW ride can OPEN into an already-rugged round — `open` aborts if the round it would join has already fired its rug, so a player can't bet INTO a known-doomed round and immediately drain against it (the post-rug open-drain vector, the open-side complement to the close/crank bounds above, v4.27 fix #758) | `open_into_an_already_rugged_round_aborts` | `segment_market_v4_tests.move` |
| Enabling the rug is admin-gated — `enable_rug` requires the market's OWN vault admin cap; a foreign/non-owning cap aborts, so an attacker can't bootstrap (front-run) a rug on a market they don't control (closes the enable_rug bootstrap front-run window, v4.27 fix #767) | `enable_rug_with_a_foreign_vault_cap_aborts` | `segment_market_v4_tests.move` |
