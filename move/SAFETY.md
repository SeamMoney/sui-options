...
Run the whole suite: `sui move test` (from `move/`) — **602/602 passing**.
...
| A touch/no-touch open is fully guarded (funded stake, before expiry, own vault + path) | `cannot_open_with_zero_stake` · `cannot_open_after_expiry` · `cannot_open_against_wrong_vault` · `cannot_open_against_wrong_path` | `market_tests.move` |
