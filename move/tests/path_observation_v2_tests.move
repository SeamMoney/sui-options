...
// === Aborted refund branch in market.move ===

#[test]
fun aborted_market_refunds_both_sides_one_to_one() {
    let mut sc = ts::begin(ALICE);
    let mut clk = clock::create_for_testing(sc.ctx());
    ...
    let mut path = po::new_v2(
        &oracle, BARRIER_ABOVE, po::touch_above(),
        0, 10, 1, 5_000, sc.ctx(),  // grace 5s
    );

    let seed = coin::mint_for_testing<SUI>(10_000_000_000, sc.ctx());
    ...
    let stake_b = coin::mint_for_testing<SUI>(1_000_000_000, sc.ctx());
    let bob_pos = market::open<SUI>(&mut mkt, market::side_no_touch(), stake_b, &clk, sc.ctx());

    // Vault now holds seed + 2 stakes
    assert!(market::vault_balance(&mkt) == 12_000_000_000, 0);

    // Push 0 observations. Move clock past expiry+grace → Aborted state.
    ...
    assert!(alice_payout.value() == 1_000_000_000, 2);
    assert!(bob_payout.value() == 1_000_000_000, 3);

    // Vault: seed remains intact (only stakes refunded).
    assert!(market::vault_balance(&mkt) == 10_000_000_000, 4);
