...
→ **One command, the COMPLETE audit of a real ride**: `npm run audit:latest` (no args — it picks the newest closed ride for you) or `npm run audit:ride -- --market <id> --ride <id>` for a specific one — runs both verifiers and passes only if all four hold — honest candles · honest house edge · correct verdict · exact payout → `✅ COMPLETE AUDIT PASS`. Every verifier prints a **SuiScan link** for the package, market, ride, and settlement tx — so you re-derive the math *and* open the raw on-chain state in one click. `npm run prove:live` does the same for all five live checks (randomness → markets → audit → halts → vaults).
→ **Run the whole on-chain loop cold** (one command, no wallet): `npm run smoke:ride` — mints a burner, funds it from the production faucet, opens → cranks → settles a real touch-either ride, then audits it with `verify-v4` (**PASS**). Every step prints a SuiScan link. (`npm run smoke:halt` drives a live **MARKET HALT** the same way, but it's **operator-run** — it needs the faucet key, since reliably catching a rug burns ~67 segments of gas, more than a cold burner's drip; a judge verifies any halt **cold, no key** via `npm run verify:halt` or [`/verify`](https://wick-markets.vercel.app/verify).)
...
The house chooses nothing: not the keys, not the barriers, not the candles, not the verdict, not the money. **And it can't change the rules after you bet** — the multiplier, rug chance, barrier offset, deadband, and spread are set once at market bootstrap with *no on-chain setter* (the only rug-chance mutator is `#[test_only]`, compiled out of the published package; traceability in [`move/SAFETY.md`](move/SAFETY.md)). Immutable rules, verifiable outputs. If a candle in your loss looks wrong, replay it. The whole ride is reproducible bit-for-bit. **→ [The provable-fairness chain, rendered](docs/provable-fairness.md)** — a diagram of every derivation with the one command that proves it. The rest of the picture, also as rendered diagrams: the [Sui-native object model + settlement state machine](CONTRACT_INTERFACE.md) and the [`/pro` pricing pipeline](DEMO.md) (live DeepBook mark → Black-Scholes → settlement-consistent P&L).
...
- **Genesis walk-state seed.** verify-v4 reads each segment's recorded `state_after` straight from the chain, so the old replay-from-k=0 seed mismatch is gone for the common case. The one residual: a *genesis* audit (segment 0 — e.g. the rug path — which has no prior segment to read) assumes the bootstrap default `vol_regime_init = 1_000_000n`; a market deployed with a non-default value would mismatch at segment 0. Every live demo market uses the default (runtime warning otherwise); the full fix is to emit `vol_regime_init` in `SegmentMarketCreated`.
...
# Move
cd move && sui move test && cd ..          # 697/697
...
move/tests/        697 Move tests including conformance, invariants, adversarial, e2e replay, v4.26 rug-roll suite
