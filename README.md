...
| Layer | Verification |
|---|---|
| `move/sources/*.move` (26 modules) | **571 / 571** Move tests pass on every commit (`sui move test`) including invariant suite, adversarial suite, spine-test-2 e2e replay, full DNT lifecycle, deadband, deferred-spread, FSM determinism, and the v4.26 rug-roll determinism + replay suite |
...
# Move
cd move && sui move test && cd ..          # 571/571
