...
→ **▶ Wick Pro — the options game** (start here): [wick-markets.vercel.app/pro](https://wick-markets.vercel.app/pro) — one-tap Black-Scholes calls & puts priced off a **live DeepBook mark** (SUI/USDC or DEEP/USDC mid, with σ from the live trade tape). Tap **UP** or **DOWN**, watch **one big live P&L** glide off the real mid at 60fps, then **CLOSE** / **FLIP** or let it auto-settle in 60s — the result equals the live number you watched, to the cent (settlement-consistent). A CandleVision pattern-coach reads the same live tape. No wallet, mobile-first. _(A synthetic, commit–reveal **provably-fair** variant lives at [`/pro-sim`](https://wick-markets.vercel.app/pro-sim).)_
