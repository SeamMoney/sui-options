/**
 * rugRoll — in-browser port of Move `segment_market_v4::roll_rug`, the v4.26
 * MARKET-HALT (rug) dice. Byte-identical to the CLI's `scripts/rugRoll.ts`
 * (a conformance test pins them together) so the /verify page can re-derive the
 * house-edge roll for any segment from its on-chain key:
 *
 ...
 *
 * The dice are public: anyone with a segment key, the market id, and the round
 * index can re-run this and confirm the chain only ever halted a round when
 * `roll < rug_chance_bps` — the provable-fairness guarantee for the house edge,
 * the same in spirit as the candle-extrema replay.
 */
