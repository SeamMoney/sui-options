...
export const DEFAULT_TA_DETECTOR_OPTIONS: Required<TaDetectorOptions> = {
  minConfidence: 0.58,
  includeWeak: false,
  fastPeriod: 20,
  slowPeriod: 50,
...
const COLORS: Record<CandleDirection, string> = {
  bullish: '#22c55e',
  bearish: '#ef4444',
  neutral: '#a78bfa',
};

function withDefaults(options: TaDetectorOptions = {}) {
...
function scoreGreater(value: number, threshold: number, fullAt: number) {
  if (value <= threshold) return 0;
  if (fullAt <= threshold) return 1;
  return clamp01((value - threshold) / (fullAt - threshold));
}

function isNumber(value: IndicatorValue): value is number {
  return typeof value === 'number' && Number.isFinite(value);
}

function candleDirection(bar: CandleInput): CandleDirection {
  if (Math.abs(bar.close - bar.open) <= 1e-9) return 'neutral';
  return bar.close > bar.open ? 'bullish' : 'bearish';
}

function makeAnchors(candles: CandleInput[], startIndex: number, endIndex: number, direction: CandleDirection): CandlePatternAnchor[] {
...
function makeEvent(params: {
  ...
  label: string;
  description: string;
  status?: CandlePatternEvent['status'];
...
function crossedAbove(previousFast: IndicatorValue, previousSlow: IndicatorValue, fast: IndicatorValue, slow: IndicatorValue) {
  return isNumber(previousFast) && isNumber(previousSlow) && isNumber(fast) && isNumber(slow) && previousFast <= previousSlow && fast > slow;
}

function crossedBelow(previousFast: IndicatorValue, previousSlow: IndicatorValue, fast: IndicatorValue, slow: IndicatorValue) {
  return isNumber(previousFast) && isNumber(previousSlow) && isNumber(fast) && isNumber(slow) && previousFast >= previousSlow && fast < slow;
}

function maCrossEvents(candles: CandleInput[], type: 'sma' | 'ema', options: TaDetectorOptions = {}) {
  ...
  for (let i = 1; i < candles.length; i += 1) {
    ...
    if (!isNumber(fastValue) || !isNumber(slowValue) || !isNumber(previousFast) || !isNumber(previousSlow)) continue;
...
export function detectSmaCrosses(candles: CandleInput[], options: TaDetectorOptions = {}) {
  return maCrossEvents(candles, 'sma', options);
}

export function detectEmaCrosses(candles: CandleInput[], options: TaDetectorOptions = {}) {
  return maCrossEvents(candles, 'ema', options);
}

export function detectRsiSetups(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
  ...
  for (let i = 1; i < candles.length; i += 1) {
    ...
    if (!isNumber(previous) || !isNumber(current)) continue;
...
function swingHighs(candles: CandleInput[], radius: number) {
  const highs: number[] = [];
  for (let i = radius; i < candles.length - radius; i += 1) {
    let isSwing = true;
    for (let j = i - radius; j <= i + radius; j += 1) {
      if (j !== i && candles[j].high >= candles[i].high) isSwing = false;
    }
    if (isSwing) highs.push(i);
  }
  return highs;
}

function swingLows(candles: CandleInput[], radius: number) {
  const lows: number[] = [];
  for (let i = radius; i < candles.length - radius; i += 1) {
    let isSwing = true;
    for (let j = i - radius; j <= i + radius; j += 1) {
      if (j !== i && candles[j].low <= candles[i].low) isSwing = false;
    }
    if (isSwing) lows.push(i);
  }
  return lows;
}

export function detectRsiDivergences(candles: CandleInput[], options: TaDetectorOptions = {}): RsiDivergenceSignal[] {
  const opts = withDefaults(options);
  const values = rsi(candles, opts.rsiPeriod);
  ...
  for (let i = 1; i < highs.length; i += 1) {
    ...
    if (!isNumber(firstRsi) || !isNumber(secondRsi)) continue;
...
export function detectMacdCrosses(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
  ...
  for (let i = 1; i < values.length; i += 1) {
    const previous = values[i - 1];
    const current = values[i];
    if (!isNumber(previous.macd) || !isNumber(previous.signal) || !isNumber(current.macd) || !isNumber(current.signal)) continue;
...
function squeezeFlags(widths: IndicatorValue[], lookback: number) {
  return widths.map((width, index) => {
    if (!isNumber(width)) return false;
    const prior = widths.slice(Math.max(0, index - lookback), index).filter(isNumber);
    if (prior.length < Math.max(6, Math.floor(lookback / 3))) return false;
    const averageWidth = prior.reduce((sum, next) => sum + next, 0) / prior.length;
    return width < averageWidth * 0.68;
  });
}

export function detectBollingerSetups(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
  ...
  for (let i = 1; i < candles.length; i += 1) {
    const band = bands[i];
    if (!isNumber(band.width) || !isNumber(band.upper) || !isNumber(band.lower)) continue;
    ...
    if (recentSqueezeIndex < 0) continue;
...
export function detectAtrExpansion(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
  ...
  for (let i = 1; i < candles.length; i += 1) {
    ...
    if (!isNumber(current) || !isNumber(currentBaseline) || !isNumber(previous) || !isNumber(previousBaseline) || currentBaseline <= 0 || previousBaseline <= 0) continue;
    const ratio = current / currentBaseline;
    const previousRatio = previous / previousBaseline;
    if (ratio < opts.atrExpansionRatio || previousRatio >= opts.atrExpansionRatio) continue;
...
export function detectVwapSetups(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
...
export function detectVolumeClimax(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
  ...
  for (let i = 1; i < candles.length; i += 1) {
    ...
    if (!isNumber(average) || average <= 0 || volume <= 0) continue;
    const ratio = volume / average;
    if (ratio < opts.volumeClimaxRatio) continue;
...
export const TA_PATTERN_DETECTORS: TaDetectorDefinition[] = [
...
function dedupe(events: CandlePatternEvent[]) {
  const selected = new Map<string, CandlePatternEvent>();
  for (const event of events) {
    const key = event.id;
    const previous = selected.get(key);
    if (!previous || event.confidence > previous.confidence) selected.set(key, event);
  }
  return Array.from(selected.values()).sort((a, b) => a.endIndex - b.endIndex || a.startIndex - b.startIndex);
}

export function detectTaPatterns(candles: CandleInput[], options: TaDetectorOptions = {}) {
  const opts = withDefaults(options);
