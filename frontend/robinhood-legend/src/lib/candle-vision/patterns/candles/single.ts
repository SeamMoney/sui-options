...
export type SingleCandleTaPatternKind =
  | 'doji'
  | 'dragonfly-doji'
  | 'gravestone-doji'
  | 'long-legged-doji'
...
export type SingleCandleTaLibCode =
  | 'CDLDOJI'
  | 'CDLDRAGONFLYDOJI'
  | 'CDLGRAVESTONEDOJI'
  | 'CDLLONGLEGGEDDOJI'
...
type PatternMeta = Omit<SingleCandleTaPatternDefinition, 'detect'>;
...
function weightedScore(parts: ScoreParts, weights?: ScoreParts) {
  const entries = Object.entries(parts);
  const totalWeight = entries.reduce((sum, [key]) => sum + (weights?.[key] ?? 1), 0);
  if (!entries.length || totalWeight <= 0) return 0;

  return clamp01(
    entries.reduce((sum, [key, value]) => sum + clamp01(value) * (weights?.[key] ?? 1), 0) / totalWeight,
  );
}

function defineSingleCandlePattern(
  meta: PatternMeta,
  evaluate: (bar: CandleFeature) => PatternEvaluation | null,
): SingleCandleTaPatternDefinition {
  return {
    ...meta,
    detect: (bar) => {
      if (bar.range <= RANGE_FLOOR) return null;

      const result = evaluate(bar);
      if (!result || result.confidence < meta.minConfidence) return null;
...
function directionalBar(bar: CandleFeature): CandleDirection | null {
  return bar.direction === 'neutral' ? null : bar.direction;
}

function directionLabel(direction: CandleDirection) {
  return direction === 'bearish' ? 'Bearish' : direction === 'bullish' ? 'Bullish' : 'Neutral';
}

function bodyCenterLocation(bar: CandleFeature) {
  return clamp01(((bodyHigh(bar) + bodyLow(bar)) / 2 - bar.low) / bar.range);
}

function bodyNearHighScore(bar: CandleFeature) {
  return scoreGreater(bodyLow(bar) <= bar.low ? 1 : (bodyLow(bar) - bar.low) / bar.range, 0.82, 0.96);
}

function bodyNearLowScore(bar: CandleFeature) {
  return scoreLess(bodyHigh(bar) >= bar.high ? 1 : (bodyHigh(bar) - bar.low) / bar.range, 0.18, 0.36);
}

function wickBalanceScore(bar: CandleFeature) {
  const largestWick = Math.max(bar.upperPct, bar.lowerPct);
  if (largestWick <= RANGE_FLOOR) return 0;
  return clamp01(1 - Math.abs(bar.upperPct - bar.lowerPct) / largestWick);
}

function openingShadowPct(bar: CandleFeature, direction: CandleDirection) {
  if (direction === 'bullish') return bar.lowerPct;
  if (direction === 'bearish') return bar.upperPct;
  return Math.min(bar.upperPct, bar.lowerPct);
}

function closingShadowPct(bar: CandleFeature, direction: CandleDirection) {
  if (direction === 'bullish') return bar.upperPct;
  if (direction === 'bearish') return bar.lowerPct;
  return Math.min(bar.upperPct, bar.lowerPct);
}
...
function dojiBodyScore(bar: CandleFeature) {
...
function dojiEvaluation(bar: CandleFeature): PatternEvaluation | null {
...
function directionalLongBodyEvaluation(
  bar: CandleFeature,
  scoreBreakdown: ScoreParts,
  weights: ScoreParts,
): { direction: CandleDirection; confidence: number } | null {
...
export const RICKSHAW_MAN_PATTERN_DEFINITION = defineSingleCandlePattern(
  ...
  (bar) => {
    ...
    const confidence = weightedScore(
      parts,
      { doji: 3, upperShadow: 2, lowerShadow: 2, centeredBody: 2, balancedShadows: 1 },
    );
    if (confidence <= 0) return null;
...
export const TAKURI_PATTERN_DEFINITION = defineSingleCandlePattern(
  ...
  (bar) => {
    ...
    const confidence = weightedScore(
      parts,
      { doji: 3, lowerShadow: 3, upperShadow: 2, bodyNearHigh: 1, range: 1 },
    );
    if (confidence <= 0) return null;
...
export const MARUBOZU_PATTERN_DEFINITION = defineSingleCandlePattern(
  ...
  (bar) => {
    ...
    const confidence = weightedScore(
      parts,
      { bodyPct: 2, bodyRatio: 1, upperShadow: 1.5, lowerShadow: 1.5 },
    );
    if (confidence <= 0) return null;
...
export const LONG_LINE_PATTERN_DEFINITION = defineSingleCandlePattern(
  ...
  (bar) => {
    ...
    const evaluation = directionalLongBodyEvaluation(
      bar,
      parts,
      { bodyPct: 2, bodyRatio: 1, upperShadow: 1, lowerShadow: 1 },
    );
    if (!evaluation || evaluation.confidence <= 0) return null;
...
export const SHORT_LINE_PATTERN_DEFINITION = defineSingleCandlePattern(
  ...
  (bar) => {
    ...
    const confidence = weightedScore(
      parts,
      { smallBody: 2, notDoji: 1, upperShadow: 1, lowerShadow: 1, compactRange: 1 },
    );
    if (confidence <= 0) return null;
...
export const SINGLE_CANDLE_TA_PATTERN_DEFINITIONS: readonly SingleCandleTaPatternDefinition[] = [
...
export function detectSingleCandleTaPatterns(
  bar: CandleFeature,
  options: SingleCandleTaDetectionOptions = {},
): SingleCandleTaPatternMatch[] {
