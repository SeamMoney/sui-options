...
const DEFAULT_STATUS_WEIGHTS: Record<CandlePatternStatus, number> = {
  confirmed: 1,
  forming: 0.82,
  invalidated: 0,
  expired: 0.16,
...
const DEFAULT_CATEGORY_WEIGHTS: Record<PatternCategory, number> = {
  candlestick: 1,
  'vision-candle': 1.08,
  'chart-pattern': 0.96,
...
function overlap(a: CandlePatternEvent, b: CandlePatternEvent) {
  return a.startIndex <= b.endIndex && b.startIndex <= a.endIndex;
}

function countBy<T extends string>(items: RankedPatternSignal[], key: (item: RankedPatternSignal) => T) {
...
function scoreSignal(
  event: CandlePatternEvent,
  definition: PatternDefinition | undefined,
  latestIndex: number,
  options: PatternSignalRankingOptions,
) {
...
function canShowSignal(
  signal: RankedPatternSignal,
  selected: RankedPatternSignal[],
  options: Required<Pick<PatternSignalRankingOptions, 'allowOverlaps' | 'perKindLimit' | 'perFamilyLimit' | 'minVisibleScore'>>,
) {
  const reasons: string[] = [];
  if (!signal.supported) reasons.push('unsupported');
  if (signal.visibleScore < options.minVisibleScore) reasons.push('below-visible-score');
  if (signal.event.status === 'invalidated') reasons.push('invalidated');
  if (signal.event.status === 'expired') reasons.push('expired');

  const kindCounts = countBy(selected, (item) => item.event.kind);
  if ((kindCounts.get(signal.event.kind) ?? 0) >= options.perKindLimit) reasons.push('kind-limit');

  const familyCounts = countBy(selected, (item) => item.event.family);
  if ((familyCounts.get(signal.event.family) ?? 0) >= options.perFamilyLimit) reasons.push('family-limit');

  if (!options.allowOverlaps && selected.some((item) => overlap(item.event, signal.event))) reasons.push('overlap');
  return reasons;
}

export function rankPatternSignals(
  ...
  const visible: RankedPatternSignal[] = [];
  ...
  for (const signal of supported.slice().sort((a, b) => b.visibleScore - a.visibleScore || b.rawScore - a.rawScore)) {
    ...
    if (visible.length >= (options.maxVisible ?? 12)) break;
  }
...
export function rankVisiblePatternSignals(
  events: CandlePatternEvent[],
  options: PatternSignalRankingOptions = {},
) {
...
export function selectVisiblePatternEvents(
  events: CandlePatternEvent[],
  options: PatternSignalRankingOptions = {},
) {
