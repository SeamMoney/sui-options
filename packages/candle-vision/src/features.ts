import type { CandleDirection, CandleFeature, CandleInput } from './types';
...
function directionOf(open: number, close: number, bodyPct: number): CandleDirection {
  if (bodyPct < 0.08) return 'neutral';
  return close > open ? 'bullish' : 'bearish';
}

function trendDirection(slope: number): CandleDirection {
  if (Math.abs(slope) < 0.001) return 'neutral';
  return slope > 0 ? 'bullish' : 'bearish';
}

export function normalizeCandles(candles: CandleInput[], contextPeriod = 20, trendPeriod = 12): CandleFeature[] {
...
export function bodyHigh(bar: CandleInput) {
  return Math.max(bar.open, bar.close);
}

export function bodyLow(bar: CandleInput) {
  return Math.min(bar.open, bar.close);
}

export function overlapsBody(a: CandleInput, b: CandleInput) {
...
export function scoreGreater(value: number, threshold: number, fullAt: number) {
  if (value <= threshold) return 0;
  if (fullAt <= threshold) return 1;
  return clamp01((value - threshold) / (fullAt - threshold));
}

export function scoreLess(value: number, threshold: number, zeroAt: number) {
  if (value <= threshold) return 1;
  if (zeroAt <= threshold) return 0;
