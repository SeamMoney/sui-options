...
import {
  walkForwardMicroBot,
  type MicroBotWalkForwardOptions,
  type MicroBotWalkForwardResult,
} from './micro-walk-forward';
...
export function evaluateMicroBotStrategy(
  candles: readonly CandleInput[],
  options: MicroBotStrategyLabOptions = {},
): MicroBotStrategyLabResult {
...
function buildVerdict(input: {
  ...
  if (input.calibration.best) reasons.push(`Best preset: ${input.calibration.best.preset.label}.`);
  ...
  if (drawdown > Math.abs(pnl) && oosTrades > 0) reasons.push('Drawdown is larger than net edge.');
