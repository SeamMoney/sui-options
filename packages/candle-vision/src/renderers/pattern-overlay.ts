...
type PatternAnimationSpec = {
  duration: number;
  strokeDash?: number[];
};

type EventVisualState = {
  phase: 'active' | 'collapsed';
  boxAlpha: number;
  labelAlpha: number;
  pinAlpha: number;
  ageMs: number;
...
type Cluster = {
  x: number;
  y: number;
  events: DrawableEvent[];
  color: string;
  direction: CandlePatternEvent['direction'];
};
...
export function resolvePatternOverlayOptions(options: PatternOverlayOptions = {}): ResolvedPatternOverlayOptions {
  return {
...
export function renderPatternOverlay(
  ...
  renderContext: PatternOverlayRenderContext,
  options: PatternOverlayOptions = {},
) {
  const resolvedOptions = resolvePatternOverlayOptions(options);
  if (!renderContext.events.length) return;
  ...
  const { boxed: active, collapsed: suppressedActive } = selectActiveBoxes(
    drawableEvents.filter((item) => item.state.phase === 'active'),
    resolvedOptions,
    renderContext.spotlightEventId,
  );
...
function selectActiveBoxes(
  items: DrawableEvent[],
  options: ResolvedPatternOverlayOptions,
  spotlightEventId?: string | null,
) {
  const maxBoxes = Math.max(0, Math.min(options.maxEvents, options.maxActiveBoxes));
  const boxed: DrawableEvent[] = [];
  const collapsed: DrawableEvent[] = [];
...
function boxesOverlapTooMuch(a: DrawableEvent, b: DrawableEvent, options: ResolvedPatternOverlayOptions) {
...
function collectDrawableEvents(
  renderContext: PatternOverlayRenderContext,
  options: ResolvedPatternOverlayOptions,
  events: readonly CandlePatternEvent[],
  now: number,
) {
  const items: DrawableEvent[] = [];
  for (const event of events) {
    ...
    if (right < -30 || left > width + 30 || bottom < -30 || top > height + 30) continue;
...
function introProgress(event: CandlePatternEvent, now: number, firstSeen?: ReadonlyMap<string, number>) {
...
function resolveEventVisualState(
  event: CandlePatternEvent,
  now: number,
  firstSeen: ReadonlyMap<string, number> | undefined,
  options: ResolvedPatternOverlayOptions,
  spotlighted: boolean,
): EventVisualState | null {
  if (spotlighted) {
  ...
  if (pinAlpha <= 0) return null;
  return { phase: 'collapsed', boxAlpha: 0, labelAlpha: 0, pinAlpha, ageMs: elapsed };
}

function eventColor(event: CandlePatternEvent, theme: CandlePatternTheme) {
...
function isComputerVisionEvent(event: CandlePatternEvent) {
  return event.family === 'vision-candle' || event.family === 'chart-setup' || event.kind.startsWith('vision-');
}

function animationSpecForPattern(event: CandlePatternEvent): PatternAnimationSpec {
  if (event.status === 'forming') return { duration: 0.9, strokeDash: [4, 7] };
  if (event.kind === 'vision-compression' || event.kind.includes('squeeze')) return { duration: 0.42, strokeDash: [5, 6] };
  if (event.kind.includes('breakout') || event.kind.includes('breakdown') || event.kind.includes('reclaim')) return { duration: 0.36 };
  if (
    event.kind.startsWith('ma-') ||
    event.kind.startsWith('rsi-') ||
    event.kind.startsWith('macd-') ||
  ...
  if (event.family === 'chart-setup') return { duration: 0.5, strokeDash: [8, 5] };
  if (event.status === 'confirmed') return { duration: 0.32 };
...
function drawBoxTag(
  ...
  if (boxWidth < 18 || boxHeight < 16) return;
...
function tagPlacementCandidates(
  renderContext: PatternOverlayRenderContext,
  options: ResolvedPatternOverlayOptions,
  tagWidth: number,
  tagHeight: number,
  ...
  const clampX = (value: number) => Math.max(8, Math.min(maxX, value));
  const clampY = (value: number) => Math.max(8, Math.min(maxY, value));
  return [
...
function clusterMarkers(items: DrawableEvent[], radius: number): Cluster[] {
...
function drawPin(ctx: CanvasRenderingContext2D, item: DrawableEvent, now: number) {
  const x = item.anchorX;
  const y = markerY(item);
...
function drawClusterBadge(ctx: CanvasRenderingContext2D, cluster: Cluster, now: number) {
...
function drawCornerBrackets(ctx: CanvasRenderingContext2D, color: string, progress: number, left: number, top: number, right: number, bottom: number) {
...
function drawConfidenceRail(ctx: CanvasRenderingContext2D, event: CandlePatternEvent, color: string, progress: number, right: number, top: number, bottom: number) {
  const height = bottom - top;
...
function drawLabel(
  ...
  if (boxes.length >= options.maxLabels) return;
...
function eventDisplayName(event: CandlePatternEvent) {
...
function shortPatternName(name: string) {
...
function isTaEvent(event: CandlePatternEvent) {
  return (
    event.kind.startsWith('ma-') ||
    event.kind.startsWith('rsi-') ||
    event.kind.startsWith('macd-') ||
