...
const AGGREGATED_COLORS: Record<CandleDirection | 'ta' | 'setup', string> = {
  bullish: '#22c55e',
  bearish: '#ef4444',
  neutral: '#facc15',
  ta: '#fb923c',
...
function makeAggregatedId(kind: string, startIndex: number, endIndex: number, prefix = 'expanded') {
  return `${prefix}:${kind}:${startIndex}:${endIndex}`;
}

function singleMatchToEvent(
  feature: CandleFeature,
  match: ReturnType<typeof detectSingleCandleTaPatterns>[number],
): CandlePatternEvent {
  return {
...
function multiMatchToEvent(
  features: CandleFeature[],
  match: ReturnType<typeof detectMultiCandlePatternMatches>[number],
): CandlePatternEvent {
  return {
...
function shiftAggregatedEvent(event: CandlePatternEvent, offset: number, allCandles: CandleInput[]) {
  if (offset === 0) return event;
  const startIndex = event.startIndex + offset;
  const endIndex = event.endIndex + offset;
  return {
