export const CANDLE_VISION_PATTERN_CATALOG = [
...
function directionalColor(direction, family) {
...
function anchorsForSpan(candles, startIndex, endIndex, direction) {
...
function spanLengthFor(entry) {
    if (entry.family === 'candlestick') {
        if (entry.kind.startsWith('three-') || entry.kind === 'morning-star' || entry.kind === 'evening-star')
            return 3;
        if (entry.kind === 'engulfing' || entry.kind === 'harami' || entry.kind === 'piercing-line' || entry.kind === 'dark-cloud-cover')
            return 2;
        return 1;
    }
    if (entry.family === 'vision-candle')
        return entry.kind === 'vision-compression' ? 7 : 5;
    if (entry.kind.includes('triangle') || entry.kind.includes('wedge') || entry.kind.includes('channel') || entry.kind.includes('head') || entry.kind === 'cup-and-handle')
        return 18;
    if (entry.kind.includes('cross') || entry.kind.startsWith('macd') || entry.kind.startsWith('bollinger'))
        return 10;
    return 8;
}
export function createPatternShowcaseEvents(candles, catalog = CANDLE_VISION_PATTERN_CATALOG) {
    if (!candles.length)
        return [];
