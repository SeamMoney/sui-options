...
export function emaValues(values, period = 20) {
    const window = sanitizePeriod(period);
...
function rsiFromAverages(averageGain, averageLoss) {
    if (averageLoss <= EPSILON && averageGain <= EPSILON)
        return 50;
    if (averageLoss <= EPSILON)
        return 100;
    if (averageGain <= EPSILON)
        return 0;
...
export function bollingerBands(candles, period = 20, multiplier = 2) {
    const window = sanitizePeriod(period);
    const output = candles.map(() => ({ middle: null, upper: null, lower: null, width: null }));
    let sum = 0;
    let sumSquares = 0;
    for (let i = 0; i < candles.length; i += 1) {
...
export function rollingVolumeAverage(candles, period = 20) {
    const window = sanitizePeriod(period);
    const output = Array(candles.length).fill(null);
    let sum = 0;
    for (let i = 0; i < candles.length; i += 1) {
        ...
        if (i >= window)
            sum -= positiveVolume(candles[i - window]);
        if (i >= window - 1)
            output[i] = sum / window;
    }
    return output;
}
export function rollingAverage(values, period = 20) {
    const window = sanitizePeriod(period);
    const output = Array(values.length).fill(null);
    const queue = [];
    let sum = 0;
