...
function directionForValue(direction) {
    return direction === 'bearish' ? -100 : 100;
}
function realBodyMidpoint(bar) {
    return (bar.open + bar.close) / 2;
}
function tolerance(bar, factor = 0.08) {
    return Math.max(bar.averageRange, bar.range, EPSILON) * factor;
}
function nearScore(a, b, amount) {
...
function bodyInside(inner, outer, padding = 0) {
    return bodyHigh(inner) <= bodyHigh(outer) + padding && bodyLow(inner) >= bodyLow(outer) - padding;
}
function bodyGapUp(a, b) {
    return bodyLow(b) > bodyHigh(a);
}
function bodyGapDown(a, b) {
    return bodyHigh(b) < bodyLow(a);
}
function trendScore(bar, direction) {
    if (direction === 'neutral')
        return 0.5;
    if (bar.trendDirection === direction)
        return 1;
    if (bar.trendDirection === 'neutral')
        return 0.55;
    return Math.abs(bar.trendSlope) < 0.004 ? 0.35 : 0;
}
function sameOpenScore(a, b) {
    return nearScore(a.open, b.open, tolerance(a, 0.06));
}
function anchorsForRange(features, startPosition, endPosition, direction) {
...
function makeCandidate(definition, features, startPosition, endPosition, direction, confidence, label, description, scoreBreakdown) {
    const start = features[startPosition];
    const end = features[endPosition];
    return {
...
const engulfing = definePattern({
    ...
    detect(features, index) {
        ...
        return makeCandidate(engulfing, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Engulfing`, engulfing.description, { engulfing: score });
    },
});
const harami = definePattern({
    ...
    detect(features, index) {
        ...
        if (!prev || !curr)
            return null;
        const direction = prev.direction === 'bearish' ? 'bullish' : prev.direction === 'bullish' ? 'bearish' : 'neutral';
        if (direction === 'neutral')
            return null;
        const score = weightedScore({
        ...
        if (score < 0.72)
            return null;
        return makeCandidate(harami, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Harami`, harami.description, { harami: score });
    },
});
const piercingLine = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(piercingLine, features, index - 1, index, 'bullish', score, piercingLine.label, piercingLine.description, { piercingLine: score });
    },
});
const darkCloudCover = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(darkCloudCover, features, index - 1, index, 'bearish', score, darkCloudCover.label, darkCloudCover.description, { darkCloudCover: score });
    },
});
const counterattack = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.72)
            return null;
        return makeCandidate(counterattack, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Counterattack`, counterattack.description, { counterattack: score });
    },
});
const dojiStar = definePattern({
    ...
    detect(features, index) {
        const prev = features[index - 1];
        const curr = features[index];
        if (!prev || !curr || prev.direction === 'neutral')
            return null;
        const direction = prev.direction === 'bearish' ? 'bullish' : 'bearish';
        ...
        if (score < 0.7)
            return null;
        return makeCandidate(dojiStar, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Doji Star`, dojiStar.description, { dojiStar: score });
    },
});
const haramiCross = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.72)
            return null;
        return makeCandidate(haramiCross, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Harami Cross`, haramiCross.description, { haramiCross: score });
    },
});
const homingPigeon = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.73)
            return null;
        return makeCandidate(homingPigeon, features, index - 1, index, 'bullish', score, homingPigeon.label, homingPigeon.description, { homingPigeon: score });
    },
});
const inNeck = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(inNeck, features, index - 1, index, 'bearish', score, inNeck.label, inNeck.description, { inNeck: score });
    },
});
const matchingLow = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.73)
            return null;
        return makeCandidate(matchingLow, features, index - 1, index, 'bullish', score, matchingLow.label, matchingLow.description, { matchingLow: score });
    },
});
const onNeck = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(onNeck, features, index - 1, index, 'bearish', score, onNeck.label, onNeck.description, { onNeck: score });
    },
});
const separatingLines = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.72)
            return null;
        return makeCandidate(separatingLines, features, index - 1, index, direction, score, `${direction === 'bullish' ? 'Bullish' : 'Bearish'} Separating Lines`, separatingLines.description, { separatingLines: score });
    },
});
const thrusting = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(thrusting, features, index - 1, index, 'bearish', score, thrusting.label, thrusting.description, { thrusting: score });
    },
});
const tasukiGap = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.72)
            return null;
        return makeCandidate(tasukiGap, features, index - 2, index, direction, score, `${direction === 'bullish' ? 'Upside' : 'Downside'} Tasuki Gap`, tasukiGap.description, { tasukiGap: score });
    },
});
const twoCrows = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(twoCrows, features, index - 2, index, 'bearish', score, twoCrows.label, twoCrows.description, { twoCrows: score });
    },
});
const upsideGapTwoCrows = definePattern({
    ...
    detect(features, index) {
        ...
        if (score < 0.74)
            return null;
        return makeCandidate(upsideGapTwoCrows, features, index - 2, index, 'bearish', score, upsideGapTwoCrows.label, upsideGapTwoCrows.description, { upsideGapTwoCrows: score });
    },
});
export const TWO_CANDLE_PATTERN_DEFINITIONS = [
    engulfing,
    harami,
    piercingLine,
    darkCloudCover,
...
export function detectTwoCandlePatternCandidatesFromFeatures(features, index, definitions = TWO_CANDLE_PATTERN_DEFINITIONS) {
    return definitions
        .filter((definition) => index >= definition.candleCount - 1)
        .map((definition) => definition.detect(features, index))
        .filter((candidate) => Boolean(candidate));
}
export function detectTwoCandlePatternCandidates(candles, options = {}) {
...
//# sourceMappingURL=two-candle.js.map