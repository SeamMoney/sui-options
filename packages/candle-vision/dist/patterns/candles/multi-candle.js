...
function matchFrom(config) {
    const confidence = weightedScore(config.parts, config.weights);
    if (confidence < (config.threshold ?? DEFAULT_THRESHOLD))
        return [];
...
function windowAt(features, endIndex, length) {
...
function bodyMid(bar) {
    return (bar.open + bar.close) / 2;
}
function bodyContains(outer, inner) {
    return bodyHigh(inner) <= bodyHigh(outer) && bodyLow(inner) >= bodyLow(outer);
}
function rangeContains(outer, inner) {
    return inner.high <= outer.high && inner.low >= outer.low;
}
function opensInsideBody(current, previous) {
    return current.open <= bodyHigh(previous) && current.open >= bodyLow(previous);
}
function bodyEngulfs(current, previous) {
    return bodyHigh(current) >= bodyHigh(previous) && bodyLow(current) <= bodyLow(previous);
}
function similarPriceScore(a, b, scale) {
    const distance = Math.abs(a - b);
    const denominator = Math.max(scale, 1e-9);
    return scoreLess(distance / denominator, 0.08, 0.22);
}
function gapUpScore(previous, current) {
    if (current.low > previous.high)
        return 1;
    if (bodyLow(current) > bodyHigh(previous))
        return 0.86;
    if (current.open > previous.close)
        return 0.58;
    return 0;
}
function gapDownScore(previous, current) {
    if (current.high < previous.low)
        return 1;
    if (bodyHigh(current) < bodyLow(previous))
        return 0.86;
    if (current.open < previous.close)
        return 0.58;
    return 0;
}
function downtrendScore(bar) {
    if (bar.trendDirection === 'bearish')
        return 1;
    if (bar.trendSlope < 0)
        return 0.76;
    return 0.42;
}
function uptrendScore(bar) {
    if (bar.trendDirection === 'bullish')
        return 1;
    if (bar.trendSlope > 0)
        return 0.76;
    return 0.42;
}
function directionRunScore(bars, direction) {
...
function threeInside(features, index) {
...
function threeOutside(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function threeLineStrike(features, index) {
...
function threeStarsInSouth(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function abandonedBaby(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function advanceBlock(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function breakaway(features, index) {
...
function concealingBabySwallow(features, index) {
    const bars = windowAt(features, index, 4);
    if (!bars)
        return [];
    const [a, b, c, d] = bars;
...
function identicalThreeCrows(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function kicking(features, index) {
...
function kickingByLength(features, index) {
    const bars = windowAt(features, index, 2);
    if (!bars)
        return [];
    const [a, b] = bars;
...
function ladderBottom(features, index) {
    const bars = windowAt(features, index, 5);
    if (!bars)
        return [];
    const [a, b, c, d, e] = bars;
...
function matHold(features, index) {
    const bars = windowAt(features, index, 5);
    if (!bars)
        return [];
    const [a, b, c, d, e] = bars;
...
function risingThreeMethods(features, index) {
    const bars = windowAt(features, index, 5);
    if (!bars)
        return [];
    const [a, b, c, d, e] = bars;
...
function fallingThreeMethods(features, index) {
    const bars = windowAt(features, index, 5);
    if (!bars)
        return [];
    const [a, b, c, d, e] = bars;
...
function stalledPattern(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function stickSandwich(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function tristar(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function uniqueThreeRiver(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function upsideGapThreeMethods(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
function downsideGapThreeMethods(features, index) {
    const bars = windowAt(features, index, 3);
    if (!bars)
        return [];
    const [a, b, c] = bars;
...
export const MULTI_CANDLE_PATTERN_DEFINITIONS = [
    {
        kind: 'three-inside',
        taLibName: 'CDL3INSIDE',
        label: 'Three Inside',
...
export function detectMultiCandlePatternMatches(features, index, definitions = MULTI_CANDLE_PATTERN_DEFINITIONS) {
    return definitions.flatMap((definition) => definition.detect(features, index));
}
//# sourceMappingURL=multi-candle.js.map