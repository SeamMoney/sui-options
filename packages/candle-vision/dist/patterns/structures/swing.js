import { clamp01, normalizeCandles, scoreGreater, scoreLess, } from '../../features.js';
const EPSILON = 1e-9;
...
const CHART_STRUCTURE_KINDS = new Set([
    'support-retest',
    'resistance-retest',
    'range-breakout',
    'range-breakdown',
...
function scoreNear(a, b, goodAt, zeroAt) {
...
function priceRange(features, startIndex = 0, endIndex = features.length - 1) {
...
function averageRange(features, startIndex, endIndex) {
    const start = Math.max(0, startIndex);
    const end = Math.min(features.length - 1, endIndex);
    if (start > end)
        return 0;
    let sum = 0;
    for (let i = start; i <= end; i += 1)
        sum += features[i].range;
    return sum / (end - start + 1);
}
function barAt(features, index) {
...
function lineValue(line, index) {
    return line.slope * index + line.intercept;
}
function trendMove(features, startIndex, endIndex) {
...
function movementScore(first, last, direction, goodAt = 0.012, fullAt = 0.06) {
...
function anchorsForSpan(features, startIndex, endIndex, direction) {
    const start = barAt(features, startIndex);
    const end = barAt(features, endIndex);
...
function anchorsForSwings(features, swings, endIndex, direction, confirmationPrice) {
...
function finalizeCandidate(features, candidate) {
    return {
        ...candidate,
...
function isRecent(endIndex, features, options) {
    return features.length - 1 - endIndex <= options.maxPatternAgeBars;
}
function passesSpan(startIndex, endIndex, options) {
    const span = endIndex - startIndex + 1;
    return span >= options.minBars && span <= options.maxBars;
}
function findBreak(features, fromIndex, toIndex, levelAt, direction, buffer) {
    const start = Math.max(0, fromIndex);
    const end = Math.min(features.length - 1, toIndex);
    for (let i = start; i <= end; i += 1) {
        const level = levelAt(i);
        if (direction === 'above' && features[i].close > level + buffer)
            return i;
        if (direction === 'below' && features[i].close < level - buffer)
            return i;
    }
    return null;
}
function uniqueSwings(swings) {
    return swings.filter((swing, index) => index === 0 || swing.index !== swings[index - 1].index || swing.kind !== swings[index - 1].kind);
}
export function extractSwings(candles, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    if (candles.length < opts.leftBars + opts.rightBars + 1)
        return [];
...
function doubleTopBottomDetector(kind) {
    ...
    return {
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (let i = 0; i <= swings.length - 3; i += 1) {
                ...
                if (!matches || !passesSpan(a.index, c.index, options))
                    continue;
                ...
                const confirmIndex = findBreak(features, c.index + 1, Math.min(features.length - 1, c.index + 18), () => b.price, bearish ? 'below' : 'above', buffer);
                const endIndex = confirmIndex ?? c.index;
                if (!isRecent(endIndex, features, options))
                    continue;
                const confirmation = confirmIndex == null ? 0.46 : 1;
                const confidence = weightedScore({ rimSimilarity, necklineDepth, separation, trend, confirmation }, { rimSimilarity: 1.35, necklineDepth: 1.15, confirmation: 1.35 });
                if (confidence < 0.58 && !options.includeWeak)
                    continue;
...
function headAndShouldersDetector(kind) {
    ...
    return {
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (let i = 0; i <= swings.length - 5; i += 1) {
                ...
                if (!matches || !passesSpan(leftShoulder.index, rightShoulder.index, options))
                    continue;
                ...
                const confirmIndex = findBreak(features, rightShoulder.index + 1, Math.min(features.length - 1, rightShoulder.index + 20), (index) => lineValue(neckline, index), inverse ? 'above' : 'below', buffer);
                const endIndex = confirmIndex ?? rightShoulder.index;
                if (!rightShoulderRespectsHead || !isRecent(endIndex, features, options))
                    continue;
                const confirmation = confirmIndex == null ? 0.42 : 1;
                const confidence = weightedScore({ shoulderSimilarity, headExtension, necklineSimilarity, balance, trend, confirmation }, { shoulderSimilarity: 1.15, headExtension: 1.3, confirmation: 1.35 });
                if (confidence < 0.6 && !options.includeWeak)
                    continue;
                candidates.push(createCandidate({
                    features,
                    kind,
...
function consolidationWindows(swings, options) {
    const windows = [];
    for (let start = 0; start < swings.length; start += 1) {
        for (let end = start + 5; end < swings.length; end += 1) {
            ...
            if (span < options.minBars)
                continue;
            if (span > options.maxBars)
                break;
...
function consolidationBreakout(features, swings, highLine, lowLine, direction, avgLocalRange) {
    const last = swings[swings.length - 1];
    const buffer = Math.max(avgLocalRange * 0.16, Math.abs(last.price) * 0.0012);
    return findBreak(features, last.index + 1, Math.min(features.length - 1, last.index + 14), (index) => direction === 'above' ? lineValue(highLine, index) : lineValue(lowLine, index), direction, buffer);
}
function triangleDetector(kind) {
    const direction = kind === 'ascending-triangle' ? 'bullish' : kind === 'descending-triangle' ? 'bearish' : 'neutral';
    return {
        kind,
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (const window of consolidationWindows(swings, options)) {
                ...
                if (firstWidth <= avgLocalRange || lastWidth <= 0)
                    continue;
                ...
                if (confidence < 0.62 && !options.includeWeak)
                    continue;
                candidates.push(createCandidate({
                    features,
                    kind,
...
function wedgeChannelDetector(kind) {
    const wedge = kind.includes('wedge');
    const rising = kind === 'rising-wedge' || kind === 'channel-up';
    const direction = kind === 'falling-wedge' ? 'bullish' : kind === 'rising-wedge' ? 'bearish' : rising ? 'bullish' : 'bearish';
    return {
        kind,
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (const window of consolidationWindows(swings, options)) {
                ...
                const firstWidth = lineValue(highLine, startIndex) - lineValue(lowLine, startIndex);
                const lastWidth = lineValue(highLine, lastIndex) - lineValue(lowLine, lastIndex);
                if (firstWidth <= avgLocalRange || lastWidth <= avgLocalRange * 0.4)
                    continue;
            ...
            for (let endIndex = Math.max(12, options.minBars); endIndex < features.length; endIndex += 1) {
                for (let flagLength = 6; flagLength <= 18; flagLength += 1) {
                    ...
                    for (let impulseLength = 5; impulseLength <= 16; impulseLength += 1) {
                        ...
                        if (impulseMove <= avgLocalRange * 2.2)
                            continue;
                        ...
                        if (!isRecent(endIndex, features, options))
                            continue;
                        const confidence = weightedScore({ impulseScore, impulseDirection, shallow, drift, flagTightness, confirmation }, { impulseScore: 1.25, impulseDirection: 1.2, confirmation: 1.15 });
                        if (confidence < 0.64 && !options.includeWeak)
                            continue;
                        candidates.push(createCandidate({
                            features,
                            kind,
    ...
    return {
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (let i = 0; i <= swings.length - 4; i += 1) {
                ...
                if (leftRim.kind !== 'high' || cupLow.kind !== 'low' || rightRim.kind !== 'high' || handleLow.kind !== 'low')
                    continue;
                const startIndex = leftRim.index;
                const endSearch = Math.min(features.length - 1, handleLow.index + 16);
                if (!passesSpan(startIndex, handleLow.index, { ...options, minBars: 24, maxBars: 110 }))
                    continue;
                ...
                const confirmIndex = findBreak(features, handleLow.index + 1, endSearch, () => rimLevel, 'above', Math.max(avgLocalRange * 0.18, rimLevel * 0.0015));
                const endIndex = confirmIndex ?? handleLow.index;
                if (!isRecent(endIndex, features, options))
                    continue;
                const confirmation = confirmIndex == null ? 0.42 : 1;
                const confidence = weightedScore({ rimSimilarity, cupDepth, lowPlacement, recovery, shallowHandle, handleAboveCupLow, confirmation }, { rimSimilarity: 1.1, cupDepth: 1.25, shallowHandle: 1.15, confirmation: 1.2 });
                if (confidence < 0.62 && !options.includeWeak)
                    continue;
                candidates.push(createCandidate({
                    features,
    ...
    return {
        ...
        detect: ({ features, swings, options }) => {
            ...
            for (let i = Math.max(8, options.minBars); i < features.length; i += 1) {
                if (!isRecent(i, features, options))
                    continue;
...
function rangeBreakDetector(kind) {
        ...
        detect: ({ features, options }) => {
            ...
            for (let i = Math.max(14, options.minBars); i < features.length; i += 1) {
                ...
                for (let length = 14; length <= 48; length += 1) {
                    ...
                    const confidence = weightedScore({ compression, breakDistance, closeLocation, expansion, volume }, { breakDistance: 1.35, closeLocation: 1.15 });
                    if (confidence < 0.64 && !options.includeWeak)
                        continue;
                    candidates.push(createCandidate({
                        features,
...
export const STRUCTURE_PATTERN_DETECTORS = [
...
function dedupe(events, maxEventsPerKind) {
    const sorted = events
...
export function detectStructurePatternsFromFeatures(features, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    if (!opts.enableChartSetups || features.length < opts.minBars)
        return [];
    const swings = extractSwings(features, opts);
    const context = { features, swings, options: opts };
    const candidates = STRUCTURE_PATTERN_DETECTORS.flatMap((detector) => {
        if (features.length < detector.minBars || swings.length < detector.minSwings)
            return [];
...
export function detectStructurePatterns(candles, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
