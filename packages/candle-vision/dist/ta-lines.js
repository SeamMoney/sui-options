...
export function deriveTaLines(candles, options = {}) {
    const opts = { ...DEFAULT_OPTIONS, ...options };
    if (candles.length < Math.max(16, opts.swingRadius * 4))
        return [];
    ...
    const lows = enforceSwingDistance(swings.filter((swing) => swing.kind === 'low').sort((a, b) => b.strength - a.strength || b.index - a.index), opts.minSwingDistance).sort((a, b) => a.index - b.index);
    const highs = enforceSwingDistance(swings.filter((swing) => swing.kind === 'high').sort((a, b) => b.strength - a.strength || b.index - a.index), opts.minSwingDistance).sort((a, b) => a.index - b.index);
...
function extractSwings(candles, radius) {
    const swings = [];
    for (let index = radius; index < candles.length - radius; index += 1) {
        ...
        const localRange = Math.max(1e-9, Math.max(bar.high, left.high, right.high) - Math.min(bar.low, left.low, right.low));
        if (high) {
...
function enforceSwingDistance(swings, minDistance) {
    const selected = [];
...
//# sourceMappingURL=ta-lines.js.map