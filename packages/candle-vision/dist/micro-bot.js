...
export function createMicroBotState(options = {}) {
    return {
...
export function updateMicroBot(input) {
    ...
    const signal = entryGate.canEnter || rawSignal.phase !== 'confirmed'
        ? rawSignal
        : {
            ...rawSignal,
            phase: entryGate.blocked ? 'blocked' : 'forming',
            reasons: prependReason(rawSignal.reasons, entryGate.reason),
        };
    let state = { ...input.state, enabled, signal };
    ...
    if (state.position) {
        ...
        if (exitReason)
            return closePosition(state, marked, latest.close, input.nowMs, exitReason, options);
...
export function setMicroBotEnabled(state, enabled) {
    return {
        ...state,
        enabled,
...
function openPosition(state, side, latest, latestIndex, nowMs, signal, options, avgRange) {
...
function closePosition(state, position, exitPrice, nowMs, exitReason, options) {
    const trade = {
        ...position,
        exitPrice,
        closedAtMs: nowMs,
...
function shouldExitPosition(position, signal, nowMs, options) {
    if (position.side === 'long' && position.markPrice >= position.targetPrice)
        return 'target';
    if (position.side === 'short' && position.markPrice <= position.targetPrice)
        return 'target';
    if (position.side === 'long' && position.markPrice <= position.stopPrice)
        return 'stop';
    if (position.side === 'short' && position.markPrice >= position.stopPrice)
        return 'stop';
    const heldMs = nowMs - position.openedAtMs;
    if (heldMs >= options.minHoldMs * 0.58 &&
        signal.side !== 'none' &&
    ...
    if (heldMs >= Math.min(2800, options.minHoldMs * 0.6) &&
        position.pnl < 0 &&
        directionalPressure < -0.24 &&
        signal.oppositeScore > 0.34) {
        return 'pressure-flip';
    }
    if (nowMs >= position.plannedExitAtMs)
        return 'time';
    return null;
}
function markPosition(position, latest, latestIndex, nowMs) {
...
function buildMicroBotSignal(candles, events, decision) {
    if (candles.length < 7)
        return FLAT_SIGNAL;
    ...
    for (const event of recentEvents) {
        ...
        if (event.family === 'chart-setup')
            setupPressure += direction * score;
        else
            patternPressure += direction * score;
    ...
    const decisionPressure = decision.side === 'long' ? decision.confidence * 0.8 :
    ...
    const side = pressure > 0.14 ? 'long' : pressure < -0.14 ? 'short' : 'none';
    for (const event of recentEvents) {
        const direction = event.direction === 'bullish' ? 1 : -1;
    ...
    const entryScore = eventMagnitude > 0.05
        ? clamp01(Math.abs(pressure) * 0.36 +
            patternScore * 0.2 +
            setupScore * 0.18 +
            momentumScore * 0.2 +
            volumeScore * 0.08 -
    ...
    const microBoundaryCrossed = tapeOverride ||
    ...
    const hardConflict = !microBoundaryCrossed &&
...
function evaluateEntryGate(signal, decision, options) {
    if (signal.side === 'none')
        return { canEnter: false, blocked: false, reason: 'No directional edge yet' };
    if (signal.phase !== 'confirmed')
        return { canEnter: false, blocked: signal.phase === 'blocked', reason: 'Waiting for confirmation' };
...
function prependReason(reasons, reason) {
    return [reason, ...reasons.filter((item) => item !== reason)].slice(0, 5);
}
function momentumPressure(candles) {
...
//# sourceMappingURL=micro-bot.js.map