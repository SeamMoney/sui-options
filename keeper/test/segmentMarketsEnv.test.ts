...
// parseSegmentMarketsEnv stamps `version` (its 4th param, default "v3") on every
// binding so the cranker can pick record_segment vs record_segment_v4. With no
// override these bindings are v3.
const V3 = "v3";
...
test("the version arg is stamped on every binding (v4 markets crank record_segment_v4)", () => {
  assert.deepEqual(
    parseSegmentMarketsEnv("0xmkt@0xpkg:0xt::tusd::TUSD", PKG, SUI, "v4"),
    [{ marketId: "0xmkt", packageId: "0xpkg", collateralType: "0xt::tusd::TUSD", version: "v4" }],
  );
});
