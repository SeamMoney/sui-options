...
export function proRoundCommit(seed: number, paramsJson: string): string {
 ...
 */
export function verifyProRound(publishedCommit: string, seed: number, paramsJson: string): boolean {
  if (typeof publishedCommit !== "string" || !/^[0-9a-fA-F]{64}$/.test(publishedCommit)) return false;
