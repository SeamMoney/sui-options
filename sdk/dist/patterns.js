 ...
 */
const trendBefore = (window, offset, lookback) => {
    if (offset < lookback)
        return 0;
...
const shadowLeqBpsOfRange = (shadow, r, bps) => {
    if (r === ZERO)
        return true;
...
// ---------------------------------------------------------------------------
const candleAt = (window, i) => {
    if (i < 0 || i >= window.length)
        return null;
    const c = window[i];
    return c ?? null;
};
const mk = (name, strength, startIndex, endIndex) => ({
    name,
    matched: true,
...
export const isDoji = (window, offset) => {
    ...
    if (r === ZERO)
        return null;
    if (!bodyLeqBpsOfRange(c, DOJI_BODY_BPS))
        return null;
...
export const isDragonflyDoji = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    ...
    const u = upper(c);
    const l = lower(c);
    if (!shadowLeqBpsOfRange(u, r, NO_WICK_BPS))
        return null;
    if (l * 2n < r)
        return null; // lower wick should dominate
    return mk("Dragonfly Doji", ratio01(l, r), offset, offset);
};
// --- 3. Gravestone Doji --------------------------------------------------
// Doji + no lower wick + long upper wick.
export const isGravestoneDoji = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    ...
    const u = upper(c);
    const l = lower(c);
    if (!shadowLeqBpsOfRange(l, r, NO_WICK_BPS))
        return null;
    if (u * 2n < r)
        return null;
...
export const isLongLeggedDoji = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isRickshawMan = (window, offset) => {
...
export const isTakuri = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isHammer = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    if (r === ZERO)
        return null;
    if (!bodyLeqBpsOfRange(c, SMALL_BODY_BPS))
        return null;
    const b = body(c);
    const u = upper(c);
    const l = lower(c);
    if (l < b * LONG_WICK_VS_BODY)
        return null;
    if (u > b)
        return null;
    // need preceding downtrend for the classical hammer
    if (trendBefore(window, offset, 3) >= 0)
        return null;
...
export const isInvertedHammer = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isHangingMan = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isShootingStar = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isMarubozu = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    if (r === ZERO)
        return null;
    if (!bodyGeqBpsOfRange(c, MARUBOZU_BODY_BPS))
        return null;
...
export const isClosingMarubozu = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    if (r === ZERO)
        return null;
    if (!bodyGeqBpsOfRange(c, LONG_BODY_BPS))
        return null;
    if (isBull(c)) {
        // close at high → upper wick ≈ 0
        if (!shadowLeqBpsOfRange(upper(c), r, NO_WICK_BPS))
            return null;
    }
    else if (isBear(c)) {
        // close at low → lower wick ≈ 0
        if (!shadowLeqBpsOfRange(lower(c), r, NO_WICK_BPS))
            return null;
    }
    else {
        return null;
...
export const isSpinningTop = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isHighWave = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
    ...
    const b = body(c);
    if (b === ZERO)
        return null; // pure doji handled elsewhere
    const u = upper(c);
    const l = lower(c);
    if (u < b * 3n)
        return null;
...
export const isLongLine = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isShortLine = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isBeltHold = (window, offset) => {
    const c = candleAt(window, offset);
    if (!c)
        return null;
    const r = range(c);
...
export const isEngulfing = (window, offset) => {
    ...
    if (isBull(prev) === isBull(cur))
        return null;
    if (body(prev) === ZERO || body(cur) === ZERO)
        return null;
    const prevTop = bodyTop(prev);
    const prevBot = bodyBot(prev);
    const curTop = bodyTop(cur);
    const curBot = bodyBot(cur);
    if (curTop < prevTop || curBot > prevBot)
        return null;
    const strength = clamp01(ratio01(body(cur) - body(prev), body(prev) === ZERO ? ONE : body(prev)));
...
export const isHarami = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    if (body(prev) === ZERO)
        return null;
    if (!bodyGeqBpsOfRange(prev, LONG_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(cur, SMALL_BODY_BPS))
        return null;
    const prevTop = bodyTop(prev);
    const prevBot = bodyBot(prev);
    if (bodyTop(cur) > prevTop || bodyBot(cur) < prevBot)
        return null;
    // exclude pure doji (Harami Cross handles that)
    if (bodyLeqBpsOfRange(cur, DOJI_BODY_BPS))
        return null;
...
export const isHaramiCross = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
...
// the previous body.
export const isPiercing = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (!bodyGeqBpsOfRange(prev, LONG_BODY_BPS))
        return null;
    if (!approxEqual(cur.open, prev.close, range(prev)))
        return null;
    const mid = bodyMid(prev);
    if (cur.close <= mid)
        return null;
    if (cur.close >= prev.open)
        return null; // would be Engulfing, not Piercing
...
export const isDarkCloudCover = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (cur.close >= mid)
        return null;
    if (cur.close <= prev.open)
        return null; // would be Engulfing
...
export const isCounterattack = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (!bodyGeqBpsOfRange(prev, LONG_BODY_BPS))
        return null;
    if (!bodyGeqBpsOfRange(cur, LONG_BODY_BPS))
        return null;
    if (!approxEqual(prev.close, cur.close, range(prev)))
        return null;
...
// trend direction.
export const isSeparatingLines = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (!bodyGeqBpsOfRange(cur, LONG_BODY_BPS))
        return null;
    if (!approxEqual(cur.open, prev.open, range(prev)))
        return null;
    ...
    if (trend === 0)
        return null;
    if (isBull(cur) && trend < 0)
        return null;
    if (isBear(cur) && trend > 0)
        return null;
...
export const isMatchingLow = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    if (!isBear(prev) || !isBear(cur))
        return null;
    if (!approxEqual(prev.close, cur.close, range(prev)))
        return null;
    if (trendBefore(window, offset, 3) >= 0)
...
export const isHomingPigeon = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (!bodyGeqBpsOfRange(prev, LONG_BODY_BPS))
        return null;
    if (bodyTop(cur) > bodyTop(prev) || bodyBot(cur) < bodyBot(prev))
        return null;
...
export const isOnNeck = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
    ...
    if (!bodyGeqBpsOfRange(prev, LONG_BODY_BPS))
        return null;
    if (!approxEqual(cur.close, prev.low, range(prev)))
        return null;
...
export const isInNeck = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
...
export const isThrusting = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
...
export const isDojiStar = (window, offset) => {
    const prev = candleAt(window, offset - 1);
    const cur = candleAt(window, offset);
    if (!prev || !cur)
        return null;
...
export const isHikkake = (window, offset) => {
    ...
    if (c1.high >= c0.high || c1.low <= c0.low)
        return null;
...
export const isMorningStar = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
        return null;
    if (!isBear(c1) || !isBull(c3))
        return null;
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(c2, SMALL_BODY_BPS))
        return null;
    // exclude doji star variant (handled separately)
    if (bodyLeqBpsOfRange(c2, DOJI_BODY_BPS))
        return null;
    // continuous form: c2 opens ≈ c1.close, c3 opens ≈ c2.close
    if (!approxEqual(c2.open, c1.close, range(c1)))
        return null;
    if (!approxEqual(c3.open, c2.close, range(c2) + ONE))
        return null;
...
export const isEveningStar = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
        return null;
    if (!isBull(c1) || !isBear(c3))
        return null;
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(c2, SMALL_BODY_BPS))
...
export const isMorningDojiStar = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
...
export const isEveningDojiStar = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
...
export const isThreeWhiteSoldiers = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
    ...
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (!bodyGeqBpsOfRange(c2, LONG_BODY_BPS))
        return null;
    if (!bodyGeqBpsOfRange(c3, LONG_BODY_BPS))
        return null;
...
export const isThreeBlackCrows = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
...
export const isIdenticalThreeCrows = (window, offset) => {
    ...
    if (!approxEqual(c2.open, c1.close, range(c1)))
        return null;
    if (!approxEqual(c3.open, c2.close, range(c2)))
        return null;
...
export const isThreeInsideUpDown = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
    ...
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (bodyTop(c2) > bodyTop(c1) || bodyBot(c2) < bodyBot(c1))
        return null;
    if (body(c2) >= body(c1))
        return null;
...
export const isThreeOutsideUpDown = (window, offset) => {
    ...
    if (!c2 || !c3)
        return null;
    if (isBull(c2) && !(isBull(c3) && c3.close > c2.close))
        return null;
    if (isBear(c2) && !(isBear(c3) && c3.close < c2.close))
        return null;
...
export const isThreeStarsInTheSouth = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
    ...
    if (c3.low < c2.low)
        return null;
    if (!shadowLeqBpsOfRange(lower(c3), range(c3) + ONE, NO_WICK_BPS))
        return null;
    if (!shadowLeqBpsOfRange(upper(c3), range(c3) + ONE, NO_WICK_BPS))
        return null;
...
export const isTristar = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
        return null;
    if (!bodyLeqBpsOfRange(c1, DOJI_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(c2, DOJI_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(c3, DOJI_BODY_BPS))
        return null;
...
// above c2.close.
export const isUniqueThreeRiver = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
    ...
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (lower(c2) < body(c2) * 2n)
        return null; // hammer-style
    if (c2.low >= c1.low)
        return null;
    if (!bodyLeqBpsOfRange(c3, SMALL_BODY_BPS))
        return null;
    if (c3.open > c2.close)
        return null;
    if (c3.close <= c2.close)
...
export const isAdvanceBlock = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
    ...
    if (body(c3) >= body(c2))
        return null;
    if (upper(c3) <= upper(c2))
        return null;
    if (upper(c2) < upper(c1))
        return null;
...
export const isStalledPattern = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
...
export const isTwoCrows = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
        return null;
    if (!isBull(c1) || !isBear(c2) || !isBear(c3))
        return null;
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (!approxEqual(c2.open, c1.close, range(c1)))
        return null;
    if (c3.open <= bodyTop(c2))
        return null;
...
export const isStickSandwich = (window, offset) => {
    const c1 = candleAt(window, offset - 2);
    const c2 = candleAt(window, offset - 1);
    const c3 = candleAt(window, offset);
    if (!c1 || !c2 || !c3)
        return null;
    if (!isBear(c1) || !isBull(c2) || !isBear(c3))
        return null;
    if (!approxEqual(c1.close, c3.close, range(c1)))
        return null;
    // c2's body must sit above the close of c1/c3
    if (bodyBot(c2) < c1.close)
        return null;
...
export const isThreeLineStrike = (window, offset) => {
...
export const isConcealingBabySwallow = (window, offset) => {
    const c1 = candleAt(window, offset - 3);
    const c2 = candleAt(window, offset - 2);
    const c3 = candleAt(window, offset - 1);
    const c4 = candleAt(window, offset);
    if (!c1 || !c2 || !c3 || !c4)
        return null;
    if (!isBear(c1) || !isBear(c2) || !isBear(c3) || !isBear(c4))
        return null;
    if (!bodyGeqBpsOfRange(c1, MARUBOZU_BODY_BPS))
        return null;
    if (!bodyGeqBpsOfRange(c2, MARUBOZU_BODY_BPS))
        return null;
    ...
    if (c3.high <= c2.close)
        return null;
    // c4 engulfs c3 (range)
    if (c4.high < c3.high || c4.low > c3.low)
        return null;
...
// replaced with "≈".
export const isLadderBottom = (window, offset) => {
...
export const isMatHold = (window, offset) => {
    const c1 = candleAt(window, offset - 4);
    const c2 = candleAt(window, offset - 3);
    const c3 = candleAt(window, offset - 2);
    const c4 = candleAt(window, offset - 1);
    ...
    if (!bodyGeqBpsOfRange(c1, LONG_BODY_BPS))
        return null;
    if (!bodyGeqBpsOfRange(c5, LONG_BODY_BPS))
        return null;
    ...
    if (!bodyLeqBpsOfRange(c3, SMALL_BODY_BPS))
        return null;
    if (!bodyLeqBpsOfRange(c4, SMALL_BODY_BPS))
        return null;
    // Stay above c1.low
    if (c2.low <= c1.low)
        return null;
    ...
    if (c3.close >= c2.close || c4.close >= c3.close)
        return null;
    // breakout
...
export const isRisingFallingThreeMethods = (window, offset) => {
    const c1 = candleAt(window, offset - 4);
    const c2 = candleAt(window, offset - 3);
    const c3 = candleAt(window, offset - 2);
    const c4 = candleAt(window, offset - 1);
    ...
    if (c2.high > top || c2.low < bot)
        return null;
    if (c3.high > top || c3.low < bot)
        return null;
    if (c4.high > top || c4.low < bot)
        return null;
...
// below c2.open.
export const isBreakaway = (window, offset) => {
    const c1 = candleAt(window, offset - 4);
    const c2 = candleAt(window, offset - 3);
    const c3 = candleAt(window, offset - 2);
    const c4 = candleAt(window, offset - 1);
...
export const isModifiedHikkake = (window, offset) => {
    ...
    if (!setup || !outer || !inside || !conf)
        return null;
    // setup is small-bodied
    if (!bodyLeqBpsOfRange(setup, SMALL_BODY_BPS))
        return null;
    // inside-bar relationship
    if (inside.high >= outer.high || inside.low <= outer.low)
        return null;
...
const _PATTERN_COVERAGE_CHECK = true;
 ...
 * are immutable).
 */
export const detectPatterns = (window) => {
    const matches = [];
 ...
 */
export const detectPatternsAt = (window, offset) => {
    const matches = [];
    for (const det of DETECTORS) {
        if (offset < det.size - 1)
            continue;
...
const patternSlug = (name) => name.toLowerCase().replace(/[^a-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
/**
 ...
 */
export const detectPostHocPattern = (candles) => {
    if (candles.length === 0)
        return [];
