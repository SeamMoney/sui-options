...
const VR_MIN = 250000n;
const VR_MAX = 3600000n;
const C_VR_JITTER = 300000n;
const C_VR_JUMP_PROB = 40000n;
const C_VR_JUMP = 3000000n;
const C_VR_REVERT = 45000n;
const C_MOM_JITTER = 2600n;
const C_MOM_DECAY = 820000n;
const C_MOM_CAP = 13000n;
const C_REVERT = 7000n;
const C_VOL = 6000n;
const VOL_FLOOR = 400000n;
const C_FAT_PROB = 70000n;
const C_FAT_BASE = 2500000n;
const C_MAX_DELTA = 60000n;
...
const FSM_ENTRY_PROB = 50000n; // 5% per NORMAL candle
const W_DOJI = 30n;
const W_HAMMER = 20n;
...
const C_U = 5000n; // 0.005 * price * vr
const U_FLOOR = 400000n; // $0.40
const MOM_LEGACY_U_MUL = 250000n; // 0.25 * U
/**
...
export const REGIME_RANGE = 0;
export const REGIME_TREND_UP = 1;
export const REGIME_TREND_DOWN = 2;
 ...
 */
export function regimeDriftForRound(marketId, roundIndex) {
 ...
 */
export function applyCumulativeDrift(baseline, drift, segOffset) {
    if (drift.magBps === 0n || segOffset === 0n)
        return baseline;
...
export function detectArmedPattern(walkState) {
    const patternId = Number(walkState.patternId);
    if (patternId === PATTERN_NONE)
        return null;
    ...
    const phase = candlesRemaining <= 0
        ? "fired"
        : total === 1
            ? "firing"
...
// ── Signed arithmetic ───────────────────────────────────────────────────────
export const sZero = () => ({ neg: false, mag: 0n });
/** Canonical constructor: zero is always non-negative. */
export const sNew = (neg, mag) => ({
    neg: neg && mag !== 0n,
    mag,
});
const sPos = (mag) => ({ neg: false, mag });
/** a + b */
export function sAdd(a, b) {
    if (a.neg === b.neg)
        return sNew(a.neg, a.mag + b.mag);
    if (a.mag >= b.mag)
        return sNew(a.neg, a.mag - b.mag);
    return sNew(b.neg, b.mag - a.mag);
}
/** a - b */
export const sSub = (a, b) => sAdd(a, sNew(!b.neg, b.mag));
/** (a.mag * mul / div) carrying a's sign. */
export const sMulDiv = (a, mul, div) => sNew(a.neg, (a.mag * mul) / div);
/** Clamp |a| to at most maxMag. */
export const sClampMag = (a, maxMag) => a.mag > maxMag ? sNew(a.neg, maxMag) : a;
/** price + delta, floored at MIN_PRICE — never underflows. */
function sApply(price, delta) {
    if (!delta.neg)
...
const centered = (d) => d >= HALF ? sPos(d - HALF) : sNew(true, HALF - d);
/** Clamp a Signed into [lo,hi] (a negative value -> lo). */
function clampToU64(s, lo, hi) {
    if (s.neg)
        return lo;
    const m = s.mag;
 ...
 */
export function keystreamWord(key, n) {
...
export const draw = (key, n) => keystreamWord(key, n) % DRAW_DEN;
// ── The walk ────────────────────────────────────────────────────────────────
/**
...
function patternTotalCandles(pid) {
    if (pid === BigInt(PATTERN_DOJI))
        return 1n;
    if (pid === BigInt(PATTERN_HAMMER))
        return 1n;
    if (pid === BigInt(PATTERN_SHOOTING_STAR))
        return 1n;
    if (pid === BigInt(PATTERN_BULLISH_ENGULFING))
        return 2n;
    if (pid === BigInt(PATTERN_BEARISH_ENGULFING))
        return 2n;
    return 3n; // PATTERN_THREE_WHITE_SOLDIERS
}
/** Build a Candle from open/close and wick magnitudes. */
function ohlc(open, close, lowerWick, upperWick) {
...
const addSigned = (p, delta) => sApply(p, delta);
// ── Hero shapers — byte-identical mirrors of seeded_path.move ──────────────
function shapeDoji(st, key, sbase) {
...
function shapeHammer(st, key, sbase) {
    const u = unitU(st.price, st.volRegime);
    const open = st.price;
...
function shapeShootingStar(st, key, sbase) {
    const u = unitU(st.price, st.volRegime);
    const open = st.price;
    const body = rngRange(key, sbase, (u * 30n) / 100n, (u * 80n) / 100n);
...
function shapeBullishEngulfing(st, key, sbase, stepIdx) {
    const u = unitU(st.price, st.volRegime);
    if (stepIdx === 0n) {
        const open = st.price;
    ...
    const body2 = rngRange(key, sbase, body1 + (u * 50n) / 100n, body1 + (u * 250n) / 100n);
    const wickLo = rngRange(key, sbase + 1, 0n, (u * 30n) / 100n);
    const wickHi = rngRange(key, sbase + 2, 0n, (u * 30n) / 100n);
...
function shapeBearishEngulfing(st, key, sbase, stepIdx) {
    const u = unitU(st.price, st.volRegime);
    if (stepIdx === 0n) {
        const open = st.price;
        const body1 = rngRange(key, sbase, (u * 30n) / 100n, (u * 80n) / 100n);
...
function threeSoldiersBodyAt(key, sbase, u, stepIdx) {
    if (stepIdx === 0n) {
        return rngRange(key, sbase, (u * 40n) / 100n, (u * 70n) / 100n);
    }
    if (stepIdx === 1n) {
        const priorBody = rngRange(key, sbase - SHAPER_STRIDE, (u * 40n) / 100n, (u * 70n) / 100n);
        return rngRange(key, sbase, priorBody + (u * 20n) / 100n, priorBody + (u * 50n) / 100n);
    }
    const priorBody = threeSoldiersBodyAt(key, sbase - SHAPER_STRIDE, u, 1n);
    return rngRange(key, sbase, priorBody + (u * 20n) / 100n, priorBody + (u * 50n) / 100n);
}
function shapeThreeWhiteSoldiers(st, key, sbase, stepIdx) {
    const u = unitU(st.price, st.volRegime);
    ...
    if (stepIdx === 0n) {
        ...
        body = rngRange(key, sbase, priorBody + (u * 20n) / 100n, priorBody + (u * 50n) / 100n);
    }
    else {
        const priorBody = threeSoldiersBodyAt(key, sbase - SHAPER_STRIDE, u, 1n);
        const halfPb = priorBody / 2n;
...
function shapeDispatch(st, key, ci) {
    ...
    let c;
    if (pid === BigInt(PATTERN_DOJI))
        c = shapeDoji(st, key, sbase);
    else if (pid === BigInt(PATTERN_HAMMER))
        c = shapeHammer(st, key, sbase);
    else if (pid === BigInt(PATTERN_SHOOTING_STAR))
        c = shapeShootingStar(st, key, sbase);
        ...
        c = shapeThreeWhiteSoldiers(st, key, sbase, stepIdx);
    st.candlesRemaining = cr - 1n;
    if (st.candlesRemaining === 0n)
        st.patternId = BigInt(PATTERN_NONE);
    return c;
}
/**
 ...
 */
export function expandSegment(state, key) {
    const st = {
        price: state.price,
...
export const newState = (price, volRegime, home) => ({
    price,
    momentum: sZero(),
    volRegime,
    home,
 ...
 */
export const stateWith = (price, momNeg, momMag, volRegime, home) => ({
    price,
 ...
 */
export const stateWithPattern = (price, momNeg, momMag, volRegime, home, patternId, candlesRemaining) => ({
    price,
    momentum: sNew(momNeg, momMag),
    volRegime,
    home,
...
//# sourceMappingURL=seededPath.js.map