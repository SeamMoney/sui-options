 ...
 */
export const TOUCHED_UPPER = 0;
export const TOUCHED_LOWER = 1;
export const TOUCHED_NONE = 2;
/** Settlement-kind values — mirror the v3 module so callers can share parsers. */
export const SETTLEMENT_OPEN_V4 = 0;
export const SETTLEMENT_TOUCH_WIN_V4 = 1;
export const SETTLEMENT_CASHOUT_V4 = 2;
/**
 ...
 */
export const SETTLEMENT_EXPIRED_LOSS_V4 = 3;
export const SETTLEMENT_ABORTED_REFUND_V4 = 4;
 ...
 */
export function buildBootstrapSegmentMarketV4Tx(a) {
 ...
 */
export function buildRecordSegmentV4Tx(a) {
    const tx = new Transaction();
    tx.setSender(a.sender);
    tx.moveCall({
 ...
 */
export function buildOpenSegmentRideV4Tx(a) {
    if (a.escrowMist <= 0n)
    ...
    if (isSui) {
        ...
        if (!a.escrowSourceCoinId) {
            throw new Error(`escrowSourceCoinId required for non-SUI collateral (${a.collateralType})`);
        }
        ...
        const extras = (a.additionalCoinIds ?? []).filter((id) => id !== a.escrowSourceCoinId);
        if (extras.length > 0) {
            tx.mergeCoins(escrowSource, extras.map((id) => tx.object(id)));
        }
    }
...
export function buildCloseSegmentRideV4Tx(a) {
    const tx = new Transaction();
    tx.setSender(a.sender);
 ...
 */
export function buildCrankExpiredSegmentRideV4Tx(a) {
    const tx = new Transaction();
    tx.setSender(a.sender);
...
export function buildAbortSegmentRideV4Tx(a) {
    const tx = new Transaction();
    tx.setSender(a.sender);
...
export function segmentMarketV4CreatedEventType(packageId) {
    return `${packageId}::segment_market_v4::SegmentMarketV4Created`;
}
export function roundStartedV4EventType(packageId) {
    return `${packageId}::segment_market_v4::RoundStartedV4`;
}
export function segmentRecordedV4EventType(packageId) {
    return `${packageId}::segment_market_v4::SegmentRecordedV4`;
}
export function rideOpenedV4EventType(packageId) {
    return `${packageId}::segment_market_v4::RideOpenedV4`;
}
export function rideClosedV4EventType(packageId) {
    return `${packageId}::segment_market_v4::RideClosedV4`;
}
export function rugFiredV4EventType(packageId) {
...
function asTouchedSide(v) {
    const n = asNumber(v);
    if (n === 0)
        return TOUCHED_UPPER;
    if (n === 1)
        return TOUCHED_LOWER;
    return TOUCHED_NONE;
}
...
export function parseSegmentMarketV4CreatedEvent(json) {
    return {
...
export function parseRoundStartedV4Event(json) {
    return {
        marketId: asString(json.market_id),
...
export function parseSegmentRecordedV4Event(json) {
    return {
        marketId: asString(json.market_id),
        k: asBigInt(json.k),
...
export function parseRideOpenedV4Event(json) {
    return {
 ...
 */
export function parseRugFiredV4EventJson(json) {
    return {
        marketId: asString(json.market_id),
        roundIndex: asBigInt(json.round_index),
...
function extractCollateralFromMarketV4Type(type) {
 ...
 */
export async function fetchSegmentMarketV4(client, marketId) {
 ...
 */
export async function fetchSegmentsV4(client, opts) {
...
export async function fetchSegmentRidePositionV4(client, rideId) {
    const o = await client.getObject({
        id: rideId,
        options: { showContent: true, showType: true },
    });
    if (!o.data || o.data.content?.dataType !== "moveObject")
        return null;
    const content = o.data.content;
    if (!/::segment_market_v4::SegmentRidePositionV4$/.test(content.type))
        return null;
    const f = content.fields;
 ...
 */
export function subscribeRugFiredV4(client, marketId, packageId, onEvent, options) {
    ...
    const pollOnce = async () => {
        ...
        try {
            ...
            fresh.sort((a, b) => a.segmentIndex < b.segmentIndex ? -1 : a.segmentIndex > b.segmentIndex ? 1 : 0);
...
//# sourceMappingURL=segmentMarketV4.js.map