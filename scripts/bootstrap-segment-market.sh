...
#   MAX_PAYOUT_PER_BARRIER=200000000   # 0.2 SUI per-barrier liability cap
...
MAX_PAYOUT_PER_BARRIER="${MAX_PAYOUT_PER_BARRIER:-200000000}"
...
if [ "$OPEN_WINDOW_SEGMENTS" -ge "$ROUND_DURATION_SEGMENTS" ]; then
...
# Pull the created SegmentMarket<SUI> from objectChanges. We accept either an
# exact match on the fully-qualified type tag OR a substring match on
# "::segment_market::SegmentMarket<" (defensive against type-tag normalisation
# differences across sui CLI versions).
...
for chg in d.get('objectChanges', []):
    if chg.get('type') == 'created' and needle in chg.get('objectType', ''):
        print(chg['objectId'])
        sys.exit(0)
sys.exit('no SegmentMarket<SUI> in objectChanges — check $OUT')
")
